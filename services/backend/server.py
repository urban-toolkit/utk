...
def serve_linkLayers():
    ...
    fi.addLayerFromJsonFile(os.path.join(
        workDir, inData+".json"), abstract=abstract)

    if (abstract):
        if (maxDistance != None):
            fi.attachAbstractToPhysical(
                out, inData, outLevel, inLevel, spatial_relation, operation, maxDistance, default_value=defaultValue)
        else:
            fi.attachAbstractToPhysical(
                out, inData, outLevel, inLevel, spatial_relation, operation, default_value=defaultValue)
    else:
        if (maxDistance != None):
            fi.attachPhysicalLayers(out, inData, spatial_relation,
                                    outLevel, inLevel, maxDistance, default_value=defaultValue)
        else:
            fi.attachPhysicalLayers(
                out, inData, spatial_relation, outLevel, inLevel, default_value=defaultValue)
...
def serve_solveNominatim():
    ...
    convertedProj = utils.convertProjections(
        "4326", "3395", [location.latitude, location.longitude])
