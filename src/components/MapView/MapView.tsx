...
class D3App {
  ...
  _d3Expec: D3Expec;
  _images: {htmlObject: HTMLImageElement, id: number, width: number, height: number}[] = []; // array storing information of all images created so far TODO: should the images be saved locally and erased after?
  _svgSelector: string;
  ...
  async run(data: any[], imageId: number){
    ...
    image.addEventListener('load', function() {
      ...
      _this._images.push({htmlObject: image, id: imageId, width: this.width, height: this.height});
    ...
    let mergedImages = await this.mergeImages(); // merge all created textures in the canvas

    return {imageObject: mergedImages, singleWidth: this._images[0].width, singleHeight: this._images[0].height};
  ...
  async mergeImages(){
    ...
    let canvasContext = this._mergingCanvas.getContext('2d');

    if(!canvasContext){
      throw new Error("Texture merging failed");
    }

    // the canvas dimensions are a combination of all images dimensions (considering that all images have the same size because are generated by the same D3 specifications)
    let canvasWidth = this._images[0].width*this._images.length; 
    let canvasHeight = this._images[0].height;

    this._mergingCanvas.width = canvasWidth;
    this._mergingCanvas.height = canvasHeight;

    // reset the canvas
    canvasContext.clearRect(0, 0, canvasWidth, canvasHeight);

    for(let i = 0; i < this._images.length; i++){
      canvasContext.drawImage(this._images[i].htmlObject, (i/this._images.length)*canvasWidth, 0, this._images[i].width, this._images[i].height); // all images are aligned in a straight line
    }

    let png = this._mergingCanvas.toDataURL(); // default png

    let lockFlag = new LockFlag(); // flag to indicate if the image was loaded

    // merged image from canvas to HTMLImageElement
    let image = new Image();
    image.addEventListener('load', function() {
      lockFlag.set();
    });
  ...
  /**
   * Resets all the images and merging canvas
   */
  public cleanImages(){
    this._images = [];
    ...
    if(!canvasContext){
      throw new Error("Clean images failed");
    }

    // reset the canvas
    canvasContext.clearRect(0, 0, this._mergingCanvas.width, this._mergingCanvas.height);
  }
