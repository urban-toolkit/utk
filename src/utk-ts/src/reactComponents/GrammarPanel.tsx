...
type GrammarPanelProps = {
  ...
  camera: {
    position: number[];
    direction: { right: number[]; lookAt: number[]; up: number[] };
  };
  filterKnots: number[];
  inputId: string;
  setCamera: any;
...
export const GrammarPanelContainer = ({
  ...
  const applyGrammar = async () => {
    ...
    if (d3.select("#" + linkMapAndGrammarId).property("checked")) {
      if (tempGrammarStateRef.current == "") {
        sendGrammar = addCameraAndFilter(
  ...
  const addCameraAndFilter = (
    ...
    for (const component of parsedGrammar.components) {
      // Grammar camera is the same for all map views
      if ("map" in component) {
        if (camera.position.length != 0) component.map.camera = camera;

        if (filterKnots.length != 0) component.map.filterKnots = filterKnots;
  ...
  const updateLocalNominatim = (
    ...
    setTempGrammar(
      addCameraAndFilter(grammarStateRef.current, camera, filterKnots)
    ); // overwrite previous changes with grammar integrated with camera and filter knots
  };
  ...
  const checkIfAddCameraAndFilter = (
    grammar: string,
    camera: {
      position: number[];
      direction: { right: number[]; lookAt: number[]; up: number[] };
        ...
        <>
          <div
            ...
            style={{
              overflow: "auto",
              fontSize: "24px",
              height: "max(90%,calc(100% - 40px))",
            }}
          >
            <JSONEditorReact
              content={checkIfAddCameraAndFilter(
                grammar,
                camera,
                tempGrammar,
                filterKnots
          ...
          <div
            ...
            style={{ overflow: "auto", height: "min(10%, 40px)" }}
          >
            <Button
              variant="secondary"
              id={applyGrammarButtonId}
              style={{ marginRight: "10px" }}
            >
              Apply Grammar
            </Button>
            <input
              name="linkMapAndGrammar"
              type="checkbox"
              id={linkMapAndGrammarId}
